...
const FullscreenButton = memo(({ onClick, isFullscreen }: FullscreenButtonProps) => (
  <button
    onClick={onClick}
    className="ml-4 p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
...
const FullscreenOverlay = memo(({ isFullscreen, children }: { isFullscreen: boolean; children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm flex items-center justify-center p-6">
      <div className="w-full h-full max-w-[90vw] max-h-[90vh] bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-xl border border-gray-200/50 dark:border-slate-700/50 shadow-2xl overflow-hidden">
        {children}
      </div>
    </div>
  );
...
const lineNumberStyles =
  'w-9 shrink-0 pl-2 py-1 text-left font-mono text-gray-400 dark:text-gray-500 border-r border-gray-200/50 dark:border-slate-700/50 bg-gray-50/50 dark:bg-slate-800/50';
const lineContentStyles =
  'px-1 py-1 font-mono whitespace-pre flex-1 group-hover:bg-blue-50/50 dark:group-hover:bg-blue-900/20 text-gray-700 dark:text-gray-300';
  ...
  ({
    ...
    <div className="h-full flex flex-col items-center justify-center p-4">
      <div className="text-center text-gray-500 dark:text-gray-400">
        ...
        <p className="font-medium text-gray-700 dark:text-gray-300">Files are identical</p>
        <p className="text-sm mt-1">Both versions match exactly</p>
      </div>
      <div className="mt-4 w-full max-w-2xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-xl border border-gray-200/50 dark:border-slate-700/50 overflow-hidden shadow-lg">
        <div className="p-2 text-xs font-bold text-gray-500 dark:text-gray-400 border-b border-gray-200/50 dark:border-slate-700/50 bg-gray-50/50 dark:bg-slate-800/50">
          Current Content
        </div>
...
const FileInfo = memo(
  ({
    ...
    return (
      <div className="flex items-center bg-gray-50/80 dark:bg-slate-800/80 backdrop-blur-sm p-2 text-sm text-gray-700 dark:text-gray-300 shrink-0 border-b border-gray-200/50 dark:border-slate-700/50">
        <div className="i-ph:file mr-2 h-4 w-4 shrink-0 text-blue-500" />
        ...
        <span className="ml-auto shrink-0 flex items-center gap-2">
          {hasChanges ? (
            <>
              ...
              <span className="text-gray-400 dark:text-gray-500 text-xs">{new Date().toLocaleTimeString()}</span>
            </>
          ) : (
...
export const DiffView = memo(({ fileHistory, setFileHistory }: DiffViewProps) => {
  ...
  if (!selectedFile || !currentDocument) {
    return (
      <div className="flex w-full h-full justify-center items-center bg-white/50 dark:bg-slate-900/50 text-gray-500 dark:text-gray-400">
  ...
  try {
    ...
    return (
      <div className="flex w-full h-full justify-center items-center bg-white/50 dark:bg-slate-900/50 text-red-400">
